...
class LoggerNamespace(BaseNamespace):
    ...
    def recv_connect(self):
        ...
        overall_logger.info(f'{len(overall_logger.handlers)} '
                            'handlers attached')
        overall_logger.info(f'{self.socket}')
        ...
        def gen_emit_msg(sess_hash):
            def emit_msg(logrecord):
                if (hasattr(logrecord, 'session_hash') and
                        logrecord.session_hash == self.sess_hash):
        ...
        if existing_handler is None:
            ...
            session_log_folder = os.path.join(os.getcwd(), 'models', 'session',
                                              self.sess_id)
            ...
            overall_logger.info(f'handler for session {self.sess_id} added')
